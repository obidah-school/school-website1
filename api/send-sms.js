...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!apiKey) {
      return res.status(400).json({ success: false, message: "SMS_API_KEY غير موجود في Environment Variables" });
    }
    if (!numbers || !message) {
    ...
    const attempts = [
      ...
      },
      {
        ...
        body: JSON.stringify({ token: apiKey, numbers: cleanNums, message, sender: senderName })
      },
      {
        ...
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ apiKey: apiKey, numbers: cleanNums, message, sender: senderName })
      },
      {
        label: "form POST",
        url: "https://app.mobile.net.sa/api/v1/sendSMS",
        ...
        body: "token=" + encodeURIComponent(apiKey) + "&numbers=" + encodeURIComponent(cleanNums) + "&message=" + encodeURIComponent(message) + "&sender=" + encodeURIComponent(senderName)
      },
      {
        label: "GET webservice",
    ...
    for (const attempt of attempts) {
      try {
        ...
        results.push({ label: attempt.label, status: r.status, response: text.substring(0, 200) });
        ...
        if (ok) {
          return res.status(200).json({ success: true, message: "تم الإرسال بنجاح", via: attempt.label, response: text });
        }
      } catch (err) {
    ...
    return res.status(200).json({ success: false, message: "فشل الإرسال — تحقق من API Key", attempts: results });

  } catch (err) {
    return res.status(500).json({ success: false, message: "خطأ في السيرفر: " + err.message });
  }
};
